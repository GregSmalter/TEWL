...
namespace Tewl.Tools {
	...
	public static class AssemblyTools {
		...
		public static IEnumerable<InterfaceType> CreateInstancesOfImplementations<InterfaceType>( this Assembly assembly ) =>
			assembly.getImplementations<InterfaceType>().Select( i => (InterfaceType)Activator.CreateInstance( i ) );

		/// <summary>
		/// Builds a dictionary containing all singletons in the specified assembly that implement the specified interface type.
		/// All singletons must implement an
		/// Instance property.
		/// </summary>
		...
		private static IEnumerable<Type> getImplementations<InterfaceType>( this Assembly assembly ) =>
			assembly.GetTypes().Where( i => typeof( InterfaceType ).IsAssignableFrom( i ) && !i.IsInterface );
	}
}